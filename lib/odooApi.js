...
export async function checkSupplierStock(productCode) {
  ...
  // Query thẳng stock.quant theo x_code + location ilike 'sale'
  // x_code là custom field trên stock.quant của nhà cung cấp
  ...
  return { available, onHand, reserved: 0, productId: null, quants }
}
 ...
 */
export async function getAvailableLots(productCode, limit = 10) {
  const cfg = await getOdooConfig()

  // Tìm lot theo x_code + location ilike 'sale'
  const lots = await callOdoo('stock.lot', 'search_read', [[
    ['x_code', '=', productCode],
    ['product_qty', '>', 0],
  ]], {
...
export async function autoloadKeysFromSupplier({ productCode, quantity, orderId }) {
  ...
  const { available, quants } = await checkSupplierStock(productCode)
  if (available < quantity) throw new Error(`NCC còn ${available}, cần ${quantity}`)

  const lots = await getAvailableLots(productCode, quantity)
  ...
  const productId = selected[0]?.product_id?.[0] || null
