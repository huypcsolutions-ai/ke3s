...
export async function checkSupplierStock(productCode) {
  ...
  // 1. Query stock.quant theo x_code + location ilike 'sale'
  const quants = await callOdoo('stock.quant', 'search_read', [[
    ['x_code', '=', productCode],
    ['location_id', 'ilike', 'sale'],
  ]], {
    fields: ['quantity', 'reserved_quantity', 'product_id', 'location_id'],
  }, cfg)

  if (!quants?.length) return { available: 0, onHand: 0, reserved: 0, productId: null, quants: [] }

  // Lấy product_id từ quant đầu tiên
  const productId = quants[0]?.product_id?.[0] || null
...
export async function getAvailableLots(productId, limit = 10) {
  ...
  // Tìm lot trong stock.production.lot theo product_id
  const lots = await callOdoo('stock.production.lot', 'search_read', [[
    ['product_id', '=', productId],
    ['product_qty', '>', 0],
  ]], {
    fields: ['id', 'name', 'product_qty', 'product_id'],
    order: 'id desc',
    limit,
  }, cfg)
...
export async function autoloadKeysFromSupplier({ productCode, quantity, orderId }) {
  ...
  // 1. Lấy product_id từ stock.quant
  const { available, productId } = await checkSupplierStock(productCode)
  if (!productId) throw new Error(`Không tìm thấy "${productCode}" trong stock.quant Odoo`)
  if (available < quantity) throw new Error(`NCC còn ${available}, cần ${quantity}`)

  // 2. Lấy serial keys từ stock.production.lot theo product_id
